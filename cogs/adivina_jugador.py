...
class AdivinaJugador(commands.Cog):
    ...
    async def empezar_juego(self, ctx, juego: str):
        if juego != "adivina-jugador":
            await ctx.send("Juego no reconocido. Por ahora solo tenemos `adivina-jugador`.")
            return
        ...
        for miembro in miembros:
            ...
            mensaje = f"🎯 `{miembro.display_name}` tiene que adivinar: **{jugador_asignado}**"
    ...
    async def terminar_juego(self, ctx):
        partida = self.partidas.get(ctx.guild.id)
        if not partida or not partida["activo"]:
